...
- Окружение для прикладного кода валидируется в `src/env.ts` (Zod): обязательные переменные задаются явно, без «тихих» подстановок в модулях; при несоответствии контракту процесс падает при старте (fail-fast). Полный перечень и комментарии — в `.env.example` и README.
