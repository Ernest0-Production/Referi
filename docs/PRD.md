...
**AC:**
- Пользователь может указать: имя/псевдоним, контактную информацию (мессенджер, email, ссылка), краткую биографию (≤ 1000 символов). Участие как соискателя или как автора рефералки не привязано к отдельным «ролям аккаунта»: один и тот же пользователь может и откликаться, и публиковать рефералку в рамках правил платформы.
...
- Перед удалением выполняются синхронные расчёты по активному эскроу и возвраты по неиспользованным оплаченным токенам отклика по правилам платежей; отложенные задачи SLA/BullMQ, привязанные к пользователю и его заявкам/рефералкам, отменяются.
...
*As a* реферальщик, *I want to* создать рефералку, *so that* соискатели могли откликнуться на неё.
...
*As a* соискатель, *I want to* откликнуться на рефералку, *so that* реферальщик увидел мою кандидатуру.
...
- Разовая покупка отклика снимает ограничение для одной конкретной рефералки.
- Соискатель не может повторно откликнуться на рефералку, по которой у него уже есть активная или завершённая заявка.
...
- Соискатель освобождает слот активного отклика и может откликнуться на другую рефералку.
- Пул попыток реферальщика НЕ уменьшается при отклонении на стадии `submitted`.

**US-010** — Отзыв отклика соискателем
*As a* соискатель, *I want to* отозвать свой отклик, *so that* освободить слот для другой рефералки.
...
**AC:**
- Если в течение 7 дней после появления первого отклика реферальщик не взял ни одного кандидата в рассмотрение — рефералка замораживается на 14 дней.
...
*As a* соискатель, *I want to* приобрести подписку, *so that* иметь возможность откликаться на большее число рефералок одновременно.
...
*As a* соискатель, *I want to* купить дополнительный отклик разово, *so that* откликнуться на конкретную рефералку без подписки.
...
Профили пользователей + CRUD рефералок + лента с фильтрами + форма отклика.
