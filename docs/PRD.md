...
| Доля возвратов эскроу от общего объёма транзакций                    | ≤ 10%            | через 6 месяцев             |
...
- **Инструмент**: Telegram-бот + admin-панель.
...
- Если аккаунт создан < 365 дней назад — пользователю показывается страница с объяснением ограничения и предложением оплатить разовый сбор (эквивалент $500 в рублях по курсу ЦБ РФ, фиксированный на момент релиза).
...
**AC:**
- Пользователь может указать: имя/псевдоним, контактную информацию (Telegram handle / email / ссылка), краткую биографию (≤ 1000 символов), роль (Соискатель / Реферальщик — можно обе).
...
- Если по заявке уже были внесены деньги в эскроу — они незамедлительно возвращаются соискателю.
...
**AC:**
- При отклике соискатель заполняет: контактная информация (Telegram / email, обязательно), краткая биография (≤ 1000 симв., обязательно), cover letter (≤ 300 симв., опционально).
...
#### Epic 4: Платежи и эскроу

**US-011** — Оплата вознаграждения реферальщику
*As a* соискатель, *I want to* внести оплату на эскроу-счёт, *so that* реферальщик передал моё резюме HR.
...
- Если сумма > 0 — соискатель оплачивает через ЮКассу (банковская карта).
- После успешного webhook-подтверждения от ЮКассы деньги захолдированы на счёте Referi (эскроу).
...
**AC:**
- При подтверждении соискателем принятия оффера (`offerAccepted`) сервис автоматически инициирует выплату.
- Сумма выплаты: сумма эскроу минус комиссия сервиса (10%).
- Выплата производится на карту реферальщика через ЮКасса Выплаты.
...
**AC:**
- Возврат производится автоматически в следующих случаях: SLA передачи резюме истёк (`refundedBySLA`), соискатель запросил отказ и реферальщик подтвердил (`refundedByCancelAck`), реферальщик не ответил на запрос отказа в 3 дня (`refundedByCancelAuto`), реферальщик удалил вакансию (`refundedByVacancyDeleted`), модератор решил в пользу соискателя (`refundedByModerator`).
- Срок возврата на карту: до 10 рабочих дней (зависит от банка).
...
**AC:**
- При переходе в `disputed` модератор получает уведомление в Telegram-бот.
...
- Решение необратимо; обе стороны получают уведомление.
...
- Жалоба отправляется модераторам через Telegram-бот.
- Пользователь получает подтверждение, что жалоба принята. Ответ модератора приходит в тот же Telegram-бот.
...
- С подпиской лимит активных откликов увеличивается с 2 до 5.
- Подписка оформляется через ЮКассу (рекуррентный платёж).
- При отмене подписки лимит возвращается к 2; активные сверхлимитные отклики сохраняются до завершения.
...
1. **Активная верификация работодателя** — сервис не проверяет принадлежность реферальщика к компании. Только реактивная мера: соискатель может подать жалобу через Telegram-бот после самостоятельной проверки по рабочей почте.
...
| ЮКасса Payments API | Приём платежей, холд, возврат | `payment_id`, `status`, подпись |
| ЮКасса Payouts API  | Выплаты реферальщику          | `payout_id`, реквизиты карты    |
| Telegram Bot API    | Модерация споров, жалобы      | `TELEGRAM_BOT_TOKEN`, webhook   |
| SMTP                | Транзакционные письма         | Событийные нотификации          |
...
| Платежи        | ЮКасса SDK (абстракция `PaymentProvider`)                    |
...
| Недоступность Telegram Bot API в РФ                       | Средняя     | Среднее | Использование прокси/webhook relay; fallback-уведомления по SMTP  |
| Юридические риски эскроу в РФ                             | Высокая     | Высокое | Консультация с юристом; оформление как «информационные услуги»    |
...
#### Phase 4 — Payments & Escrow (Sprint 4-5)
Интеграция ЮКасса + эскроу-поток + подписка Соискатель PRO + разовая покупка отклика.
...
Споры + модерация через Telegram-бот + admin-панель + жалобы.

#### Phase 7 — Polish & Production (Sprint 7-8)
Транзакционный SMTP + полировка UI + e2e Playwright + деплой в прод.

#### v1.1 (Post-launch)
Расширенная аналитика для реферальщика, уведомления в Telegram для всех пользователей, базовый антифрод.
