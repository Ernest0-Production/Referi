...
- Подписка оформляется через ЮKassa с сохранением метода оплаты (`save_payment_method`) и активирует период на 30 дней. Автоматическое продление следующего периода не входит в scope Referi v1.0 (п. 9 раздела Non-Goals).
...
9. **Автопродление подписки PRO** — пользователь продлевает доступ вручную; автосписание по сохранённой карте не реализуется.
...
- **Вебхуки**: ЮКасса — верификация `Authorization: Basic base64(shopId:secretKey)`; Telegram — верификация `X-Telegram-Bot-Api-Secret-Token`.
