...
**AC:**
- Пользователь может указать: имя/псевдоним, контактную информацию (мессенджер, email, ссылка), краткую биографию (≤ 1000 символов), роль (Соискатель / Реферальщик — можно обе).
...
- Решение необратимо; стороны получают уведомления по email при включённом SMTP (`FEATURE_EMAIL`).
...
| Недоступность SMTP у модераторов / пользователей              | Низкая      | Среднее | Мониторинг доставки; резервные каналы связи вне продукта                 |
