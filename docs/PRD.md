...
| Среднее время от запроса до перехода в `awaitingCompanyDecision`     | ≤ 14 дней        | через 3 месяца              |
| Доля рефералок, закрытых без нарушения SLA                            | ≥ 75%            | через 6 месяцев             |

---

## 2. User Experience & Functionality

### 2.0 Тон интерфейса (русский язык)

Обращение к пользователю в интерфейсе (подсказки, алерты, ошибки форм, пояснения к действиям) ведётся на **«ты»** с соответствующими местоимениями и неформальными повелительными формами, где это прямое указание пользователю. Формальное **«вы»** и повелительное **-те** в таких текстах не используем. Короткие подписи кнопок в инфинитиве без личного обращения («Сохранить», «Отмена») допустимы.
...
- **Боль**: получает слабых кандидатов из общего потока; нет инструмента для управления запросами; не защищён от мошенничества со стороны кандидатов.
...
**AC:**
- При нажатии «Войди через GitHub» открывается стандартный OAuth-флоу GitHub.
...
**AC:**
- Пользователь может указать: имя/псевдоним, контактную информацию (мессенджер, email, ссылка), краткую биографию (≤ 1000 символов). Участие как соискателя или как автора рефералки не привязано к отдельным «ролям аккаунта»: один и тот же пользователь может и отправлять запросы по рефералкам, и публиковать рефералку в рамках правил платформы.
- Контактная информация соискателя видна только реферальщику данной рефералки.
...
- В разделе «Настройки аккаунта» пользователь может безвозвратно удалить свой аккаунт после явного подтверждения в диалоге; удаляются связанные данные в сервисе (профиль, рефералки, заявки — в том числе заявки других пользователей на рефералки удаляемого реферера, пресеты поиска, подписка и связанные платежные записи там, где они были только у этого пользователя).
- Перед удалением выполняются синхронные расчёты по активному эскроу и возвраты по неиспользованным оплаченным токенам запроса по правилам платежей; отложенные задачи SLA/BullMQ, привязанные к пользователю и его заявкам/рефералкам, отменяются.
...
**US-003** — Создание рефералки реферальщиком
*As a* реферальщик, *I want to* создать рефералку, *so that* соискатели могли попросить её.

**AC:**
- Форма рефералки: название должности, название компании, специальность (выпадающий список: Frontend, Backend, Fullstack, iOS, Android, DevOps, QA, Data, ML/AI, Security), грейд (Junior / Middle / Senior / Lead), формат работы (Офис / Гибрид / Удалёнка), зарплатная вилка (от / до, RUB, опционально), описание (≤ 1000 симв.), сумма вознаграждения реферальщика (RUB, опционально; 0 означает бесплатный реферал).
- Реферальщик не может создать вторую активную рефералку, если уже есть одна. Кнопка «Создать рефералку» заблокирована с пояснением.
- Реферальщик не может создавать рефералки, если его глобальный пул попыток = 0. Кнопка заблокирована с пояснением и датой следующей регенерации попытки.
- Рефералка не показывает соискателям контактную информацию или имя реферальщика.

**US-004** — Лента рефералок с фильтрами
*As a* соискатель, *I want to* искать рефералки по нескольким параметрам, *so that* видеть только релевантные предложения.

**AC:**
- Лента содержит все активные рефералки с пагинацией (20 на страницу).
...
- Рефералки, замороженные по SLA или исчерпанию попыток, скрыты из ленты.
- Закрытые и удалённые рефералки не отображаются.
- Сортировка: по дате публикации (по умолчанию), по зарплате.

**US-005** — Удаление рефералки реферальщиком
*As a* реферальщик, *I want to* удалить свою рефералку, *so that* она более не была доступна.

**AC:**
- Реферальщик может удалить рефералку вручную в любой момент.
- Все активные заявки по этой рефералке (в состояниях `submitted`, `awaitingPayment`, `awaitingResumeHandoff`, `awaitingCompanyDecision`, `seekerCancelRequested`) переводятся в состояние `refundedByVacancyDeleted`.
...
- После удаления рефералка более не отображается нигде, кроме архива в admin-панели.

---

#### Epic 3: Запросы по рефералкам
...
**US-006** — Отправка запроса
*As a* соискатель, *I want to* попросить рефералку, *so that* реферальщик увидел мою кандидатуру.

**AC:**
- При отправке запроса соискатель заполняет: контактная информация (мессенджер или email, обязательно), краткая биография (≤ 1000 симв., обязательно), cover letter (≤ 300 симв., опционально).
- Соискатель не может иметь более 2 активных запросов по рефералкам одновременно (бесплатный лимит). Кнопка «Попросить рефералку» заблокирована с пояснением.
- С подпиской «Соискатель PRO» лимит расширяется до 5 активных запросов.
- Разовая покупка токена запроса снимает ограничение для одной конкретной рефералки.
- Соискатель не может повторно попросить ту же рефералку, по которой у него уже есть активная или завершённая заявка.

**US-007** — Просмотр запросов реферальщиком
*As a* реферальщик, *I want to* видеть список соискателей, отправивших запрос, *so that* выбрать кого рассматривать.
...
- При подтверждении пул попыток уменьшается на 1 (резервирование; при отмене/отказе компании — попытка не возвращается; при ручном удалении рефералки — возвращается).
...
- Соискатель освобождает слот активного запроса и может попросить другую рефералку.
- Пул попыток реферальщика НЕ уменьшается при отклонении на стадии `submitted`.

**US-010** — Отзыв запроса соискателем
*As a* соискатель, *I want to* отозвать свой запрос, *so that* освободить слот для другой рефералки.

**AC:**
- Соискатель может отозвать заявку в статусе `submitted` или `awaitingPayment` (до оплаты).
- Заявка переходит в `cancelled`.
- Слот активного запроса освобождается.
...
- Если рефералка не предполагает вознаграждения (сумма = 0) — заявка автоматически переходит в `awaitingResumeHandoff` без платежа.
...
- Рефералка автоматически удаляется.
...
- После возврата слот активного запроса соискателя освобождается.
...
**US-018** — Жалоба на рефералку или пользователя
*As a* пользователь, *I want to* пожаловаться на фейковую рефералку или мошенничество, *so that* защитить себя и других.

**AC:**
- На странице каждой рефералки есть кнопка «Пожаловаться».
- При нажатии открывается форма: причина (выпадающий список: Фейковая рефералка, Некорректное поведение, Мошенничество, Другое), комментарий (≤ 500 симв.).
- Жалоба сохраняется в системе; модераторы обрабатывают её через admin-панель.
- После отправки пользователь видит подтверждение (toast); ссылка для связи с модерацией вне приложения — в меню профиля в шапке (и при необходимости в кабинете, например у заявки), если задана `NEXT_PUBLIC_MODERATION_CONTACT_URL`.
- Если жалоба подтверждена — модератор может заблокировать рефералку или пользователя через admin-панель.

**Роль ADMIN (каталог и кабинет)**
У пользователя с ролью `ADMIN` в `staffRoles` (отдельно от сценария модератора по спорам): с публичной страницы рефералки доступны просмотр списка кандидатов и карточки заявки в режиме чтения (без действий реферальщика), а также открытие формы редактирования чужой активной или замороженной рефералки. Форма «Пожаловаться» на этой странице для ADMIN не показывается. Удаление чужой рефералки и операции по заявкам от имени реферальщика недоступны.
...
- Ручное удаление рефералки реферальщиком возвращает потраченные на её активные заявки попытки.

**US-020** — Фриз рефералки
*As a* система, *I want to* заморозить рефералку при нарушении SLA реакции, *so that* стимулировать реферальщика к активности.

**AC:**
- Если в течение 7 дней после появления первого запроса реферальщик не взял ни одного кандидата в рассмотрение — рефералка замораживается на 14 дней.
- Замороженная рефералка не отображается в ленте.
- Существующие заявки в статусе `submitted` остаются, но новые запросы заблокированы.
- Реферальщик получает уведомление о заморозке и дате разморозки.
- После разморозки рефералка снова появляется в ленте.
- Повторное нарушение SLA → повторная заморозка (кумулятивно).

**US-021** — Подписка «Соискатель PRO»
*As a* соискатель, *I want to* приобрести подписку, *so that* иметь возможность попросить больше рефералок одновременно.

**AC:**
- Подписка «Соискатель PRO» стоит 499 ₽/мес.
- С подпиской лимит активных запросов увеличивается с 2 до 5 (при статусе `ACTIVE` и неистёкшем оплаченном периоде `currentPeriodEnd`).
...
- При отмене подписки (`subscriptions.cancel`) лимит новых запросов возвращается к 2 сразу; автопродление прекращается. Активные сверхлимитные запросы сохраняются до завершения.

**US-022** — Разовая покупка токена запроса
*As a* соискатель, *I want to* купить дополнительный токен запроса разово, *so that* попросить конкретную рефералку без подписки.

**AC:**
- Стоимость одного дополнительного токена запроса: 199 ₽.
- Покупка привязана к конкретной рефералке и действительна 30 дней или до закрытия/удаления рефералки.
- После использования (отправки запроса) токен расходуется.
- Возврат за неиспользованный токен: если рефералка удалена — токен возвращается как деньги.
...
| ЮKassa Payments API | Обычные платежи (регистрация, подписка, токены запроса), не заявка | `payment_id`, `status`          |
...
- **Изоляция данных**: контакты соискателя доступны только реферальщику конкретной рефералки и только после того, как заявка находится в активном статусе.
...
Профили пользователей + CRUD рефералок + лента с фильтрами + форма запроса рефералки.

#### Phase 3 — Application Lifecycle Core (Sprint 3-4)
Машина состояний заявки + лимиты + запрет показа данных реферальщика + `seekerCancelRequested`.

#### Phase 4 — Payments & Safe deal (Sprint 4-5)
Интеграция ЮKassa: **безопасная сделка** по заявкам с вознаграждением, подписка Соискатель PRO, разовая покупка токена запроса (обычные платежи вне сделки).

#### Phase 5 — SLA & Automation (Sprint 5-6)
BullMQ-таймеры + авто-санкции + фриз рефералок + глобальный пул попыток + 60-дневная регенерация + авто-удаление рефералки при успехе.
