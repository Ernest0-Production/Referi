...
- [ ] `GET /` и `GET /vacancies` отображают каталог вакансий; фильтры, поиск и пагинация **не меняют** строку адреса; данные ленты — **`trpc.vacancies.list`** на клиенте с гидратацией из первого SSR; без полной перезагрузки документа
