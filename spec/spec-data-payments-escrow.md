...
**Состояние реализации**: код в репозитории может временно опираться на классический двухшаговый платёж Payments API (`capture: false` → `capture`) и отдельные выплаты без объектов сделки Safe deal. Эта спецификация задаёт **целевой контракт** и терминологию; миграция HTTP-слоя на эндпоинты Safe deal — отдельная задача без изменения машины состояний заявки.

---

## 1. Purpose & Scope

**Назначение**: определить контракты платёжного уровня приложения так, чтобы провайдер (ЮKassa) мог эволюционировать от упрощённого холда к полноценному Safe deal без переписывания бизнес-логики заявки.
...
| **Холд (Hold)**     | В промежуточной реализации — блокировка суммы на карте заказчика без списования на счёт Referi (`capture: false`) |
| **Capture**         | В промежуточной реализации — подтверждение списания; в целевой модели Safe deal соответствует шагу закрытия сделки в пользу исполнителя согласно API |
