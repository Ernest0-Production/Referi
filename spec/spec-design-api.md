...
| `vacancies.create`     | mutation | isAuth | `{ title, companyName, specialty, grade, workFormat, salaryCurrency?, salaryFrom?, salaryTo?, description, rewardKopecks }`                           | Создать вакансию; guard: 1 активная вакансия, пул попыток > 0; `salaryFrom` / `salaryTo` — неотрицательные целые (рубли), при указании обоих должно быть `salaryFrom < salaryTo`; `rewardKopecks` — целые копейки, 0…10 000 000 (до 100 000 ₽), кратны 1 000 000 (шаг 10 000 ₽ в UI)                                                                                                                                                                                                    |
| `vacancies.update`     | mutation | isAuth | `{ id, title, companyName, specialty, grade, workFormat, salaryCurrency?, salaryFrom?, salaryTo?, description, rewardKopecks }`                       | Обновить свою вакансию в статусе ACTIVE или FROZEN; те же правила для зарплатных полей, что у `create`; ограничения на `rewardKopecks` — как у `create`                                                                                                                                                                                                               |
