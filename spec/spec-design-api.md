...
| Процедура              | Тип      | Auth   | Входные данные                                                                                                                                        | Описание                                                                                                                                                                                                                                                         |
| ---------------------- | -------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vacancies.list`       | query    | public | `{ specialty?: Specialty[], grade?: Grade[], workFormat?: WorkFormat[], salaryCurrency?: SalaryCurrency, salaryFrom?, query?, sort?, page?, limit? }` | Лента активных вакансий с фильтрами; при `salaryFrom` без `salaryCurrency` подразумевается `RUB`; `salaryFrom` — целое 1…2 000 000 (та же единица, что у вакансий в выбранной валюте); нижняя граница сравнивается только с вакансиями в выбранной валюте; сортировка `salary_desc` по сырым минорным суммам — при смешанных валютах порядок условный; в строке запроса каталога устаревший токен `MOBILE` в CSV `specialty` трактуется как выбор `IOS_MOBILE` и `ANDROID_MOBILE` |
| `vacancies.getById`    | query    | public | `{ id }`                                                                                                                                              | Детальная страница вакансии (без данных реферальщика); при авторизованном вызове в ответе `isMine: boolean` — вакансия текущего пользователя (referrer); состав полей как у элемента ленты, включая `applicationCount`                                                                                                                                                                            |
| `vacancies.create`     | mutation | isAuth | `{ title, companyName, specialty, grade, workFormat, salaryCurrency?, salaryFrom?, salaryTo?, description, rewardKopecks }`                           | Создать вакансию; guard: 1 активная вакансия, пул попыток > 0; `description` — 10…1000 символов; `salaryFrom` / `salaryTo` — неотрицательные целые в выбранной `salaryCurrency` (без пересчёта курса), каждое не больше 2 000 000, при указании обоих должно быть `salaryFrom < salaryTo`; `rewardKopecks` — целые копейки, 0…10 000 000 (до 100 000 ₽), кратны 1 000 000 (шаг 10 000 ₽ в UI)                                                                                                                                                                                                    |
| `vacancies.update`     | mutation | isAuth | `{ id, title, companyName, specialty, grade, workFormat, salaryCurrency?, salaryFrom?, salaryTo?, description, rewardKopecks }`                       | Обновить свою вакансию в статусе ACTIVE или FROZEN; те же правила для зарплатных полей и `description`, что у `create`; ограничения на `rewardKopecks` — как у `create`                                                                                                                                                                                                               |
| `vacancies.delete`     | mutation | isAuth | `{ id }`                                                                                                                                              | Удалить вакансию; cascade refund                                                                                                                                                                                                                                 |
| `vacancies.myActive`   | query    | isAuth | —                                                                                                                                                     | Активная вакансия текущего реферальщика (в т.ч. `description` для формы редактирования)                                                                                                                                                                            |
| `vacancies.applicants` | query    | isAuth | `{ vacancyId }`                                                                                                                                       | Заявки на свою вакансию: `status`, соискатель, `contactInfo` / `bio` / `coverLetter`, SLA-дедлайны (`paymentDeadline`, `resumeHandoffDeadline`, `cancelAckDeadline`, `companyDecisionDeadline`), `auditLogs` (`fromStatus`, `toStatus`, по `createdAt`) для визуализации этапов; на `/vacancies/[id]` для автора/админа: если есть заявка в одном из статусов активного рассмотрения реферальщика (`AWAITING_PAYMENT`, `AWAITING_RESUME_HANDOFF`, `SEEKER_CANCEL_REQUESTED`, `AWAITING_COMPANY_DECISION`, `DISPUTED`), блок показывает только её как «Текущий Кандидат» с диаграммой этапов (пройдено / сейчас / ожидается), иначе — список с числом откликов                                                                                                                                                     |
...
Ответ `vacancies.getById` для активной вакансии содержит тот же состав полей, что элемент ленты (включая `applicationCount`). На публичной странице `/vacancies/[id]` число заявок не показывается бэджем в шапке карточки; для посетителя и соискателя (не автор вакансии) под основной CTA в футере выводится краткая текстовая подсказка по `applicationCount` (0 — «Ты будешь первым! Никто еще не откликивался», иначе — склоняемая строка вида «N человек уже запросили рефералку»).
...
- **AC-004**: Given `vacancies.getById` для активной вакансии, When возвращён ответ, Then в нём отсутствуют поля `referrerName`, `referrerContact`, `referrerId`, а также не раскрывается идентификатор реферальщика иным полем (кроме флага `isMine` для текущей сессии).
