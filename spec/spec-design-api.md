...
| Процедура              | Тип      | Auth   | Входные данные                                                                                                                                        | Описание                                                                                                                                                                                                                                                         |
| ---------------------- | -------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vacancies.list`       | query    | public | `{ specialty?: Specialty[], grade?: Grade[], workFormat?: WorkFormat[], salaryCurrency?: SalaryCurrency, salaryFrom?, query?, sort?, page?, limit? }` | Лента активных вакансий с фильтрами; при `salaryFrom` без `salaryCurrency` подразумевается `RUB`; нижняя граница сравнивается только с вакансиями в выбранной валюте; сортировка `salary_desc` по сырым минорным суммам — при смешанных валютах порядок условный; в строке запроса каталога устаревший токен `MOBILE` в CSV `specialty` трактуется как выбор `IOS_MOBILE` и `ANDROID_MOBILE` |
