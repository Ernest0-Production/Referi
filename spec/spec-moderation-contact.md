...
- После успешной отправки жалобы на рефералку пользователю показывается **toast** (Sonner) с подтверждением и опционально ссылка «Связаться с модерацией», если переменная задана ([`src/app/vacancies/[id]/ReportVacancyForm.tsx`](../src/app/vacancies/[id]/ReportVacancyForm.tsx)).
