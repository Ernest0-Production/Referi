...
**Side effects отдельно от транзакции**: вызовы внешних сервисов (ЮКасса, SMTP) происходят после успешного коммита транзакции. При сбое внешнего сервиса — повтор через BullMQ.
